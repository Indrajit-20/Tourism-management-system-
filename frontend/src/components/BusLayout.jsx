...
const BusLayout = ({ seatLayout, bookedSeats, selectedSeats, onSeatClick, busType }) => {
  ...
  const cols = [...new Set(seatLayout.map(s => s.column))].sort((a, b) => a - b);
  ...
  const renderNormalSeat = (row, col) => {
    ...
    if (!seat) return (
      <div key={`empty-${row}-${col}`} style={{ width: 50, height: 50, margin: 4, display: "inline-block" }} />
    );
    const isBooked   = bookedSeats.includes(seat.seat_number);
    const isSelected = selectedSeats.includes(seat.seat_number);
    return (
      <NormalSeat key={seat.seat_number} seat={seat} isBooked={isBooked} isSelected={isSelected} onSeatClick={onSeatClick} />
    );
  };
  ...
  const upperSeats = seatLayout.filter(s =>
    s.seat_number?.startsWith("U") || s.type === "upper"
  );
  const lowerSeats = seatLayout.filter(s =>
    s.seat_number?.startsWith("L") || s.type === "lower"
  );
  ...
  const renderSleeperSeat = (row, col, deckSeatMap) => {
    ...
    if (!seat) return (
      <div key={`empty-${row}-${col}`} style={{ width: 120, height: 42, margin: "3px 4px", display: "inline-block" }} />
    );
    const isBooked   = bookedSeats.includes(seat.seat_number);
    const isSelected = selectedSeats.includes(seat.seat_number);
    return (
      <SleeperSeat key={seat.seat_number} seat={seat} isBooked={isBooked} isSelected={isSelected} onSeatClick={onSeatClick} />
    );
  };
              ...
              <div className="me-2">
                {deckRows.map(row => (
                  <div key={`${deckLabel}-left-${row}`} className="d-flex">
                    {deckLeftCols.map(col => renderSleeperSeat(row, col, deckSeatMap))}
                  </div>
                ))}
              </div>
              ...
              <div className="ms-2">
                {deckRows.map(row => (
                  <div key={`${deckLabel}-right-${row}`} className="d-flex">
                    {deckRightCols.map(col => renderSleeperSeat(row, col, deckSeatMap))}
                  </div>
                ))}
              </div>
  ...
  return (
    <div className="bg-light p-4 rounded shadow-sm border">
      ...
      <div className="text-center mb-3 pb-2 border-bottom">
        ...
        <span className={`badge ${isSleeper ? "bg-primary" : "bg-secondary"}`}>
          {isSleeper ? "🛏 Sleeper Bus" : "🪑 Seater Bus"}
        </span>
      </div>
      ...
      {isSleeper ? (
        <div>
          {hasULPrefix ? (
            ...
            <>
              {upperSeats.length > 0 && renderDeck(
                "Upper",
              ...
              {lowerSeats.length > 0 && renderDeck(
                "Lower",
        ...
        <div className="d-flex justify-content-center align-items-start">
          ...
          <div style={{ width: 2, alignSelf: "stretch", borderLeft: "2px dashed #ccc", minHeight: 40, marginTop: 4 }} />
          <div className="ms-3">
            {rows.map(row => (
      ...
      <div className="d-flex justify-content-center gap-3 mt-3 pt-3 border-top flex-wrap">
        <div className="d-flex align-items-center gap-1">
          <div style={{ width: 28, height: 20, background: "#fff", border: "1px solid #dee2e6", borderLeft: "4px solid #dee2e6", borderRadius: 4 }} />
          <small className="text-muted">Available</small>
        </div>
        <div className="d-flex align-items-center gap-1">
          <div style={{ width: 28, height: 20, background: "#e8f5e9", border: "1px solid #198754", borderLeft: "4px solid #198754", borderRadius: 4 }} />
          <small className="text-muted">Selected</small>
        </div>
        <div className="d-flex align-items-center gap-1">
          <div style={{ width: 28, height: 20, background: "#f0f0f0", border: "1px solid #dee2e6", borderLeft: "4px solid #e91e8c", borderRadius: 4 }} />
      ...
      <div className="text-center mt-2">
        <small className="text-muted">
          💡 Front rows = higher price &nbsp;·&nbsp; Middle rows = normal &nbsp;·&nbsp; Back rows = lower price
      ...
      <div className="text-center mt-2 pt-2 border-top">
        <small>
          ...
          &nbsp;|&nbsp;
          Booked: <strong className="text-danger">{bookedSeats.length}</strong>
          &nbsp;|&nbsp;
          Available: <strong className="text-success">{seatLayout.length - bookedSeats.length}</strong>
        </small>
      </div>
